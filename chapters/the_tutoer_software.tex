...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In order to test the approach described in Chapter~\ref{ch_approach} a
...
monitor the experiment while it was running. It contains the following views.
\begin{description}
	\item[Log] A long list of logged events, as described in
		Section~\ref{sec:software_logging_module}.
	\item[Student] A list of the logged events related to a particular student
		and an overview of the test scores, assigned student group and
		presented sequence for each knowledge component the student
		participated in so far.
	\item[Population] A list of the individuals in each generation for a
		specific population, including observed fitness values, and a graph
		displaying the cumulative regret for that population.
\end{description}
